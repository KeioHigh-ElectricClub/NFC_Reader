...
void setup() {
  ...
  delay(10);
  mfrc522.PCD_Init();
  mfrc522.PCD_DumpVersionToSerial();   //0x12が出ればRC522はArduinoに認識されている
...
void loop() {
  ...
  bool Deduct  = digitalRead(3);
  bool Add     = digitalRead(4);
  bool submit  = digitalRead(5);


  //モードの判定文
  if (mode < 250) PDis("READER", mode);
  else if (450 < mode) PDis("RESET", mode);
  ...
  if (state == 510) {
    ...
    mp3_set_volume (10);              //0~30で設定。get_state()よりも後に書かなければならない
    mp3_play (2);
    delay (100);
  }
...
void Select() {
  ...
  while (phase < 5) {
    ...
    while (phase == 4) {
      ...
      if ( ! mfrc522.PICC_IsNewCardPresent()) continue;         // Mifareカードの確認（新しいカードが無ければ終了し、loop関数を繰り返す）
      if ( ! mfrc522.PICC_ReadCardSerial()) continue;           // Mifareカードのデータ読み込み（読み取れなければ終了し、loop関数を繰り返す）
      auth();
...
void BackSelect() {
  ...
  while (submit == 1) {
    Deduct = digitalRead(3);
    submit = digitalRead(5);
    if (Deduct == 0) {
      phase++;
      mp3_play (4);
      delay(100);
...
void NFCWrite(int CR, int CP, byte BN) {         //クレジット、ポイント、ブース番号
  ...
  if (piccType != MFRC522::PICC_TYPE_MIFARE_UL) {
    ...
    if (status != MFRC522::STATUS_OK) {
      digitalWrite(LED, LOW);
      mp3_play(5);
      MDis("WRITE(),PCD_Authenticate", "status error",  "Card is distant", "one more prease");
      delay(1500);
      return;
    }
  }
  ...
  if (status != MFRC522::STATUS_OK) {
    digitalWrite(LED, LOW);
    mp3_play(5);
    MDis("WRITE(),MIFARE_Write", "status error", "Write failed", "one more prease");
    delay(1500);
    return;
  }
